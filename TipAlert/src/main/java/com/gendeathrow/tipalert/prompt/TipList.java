...
import com.gendeathrow.tipalert.prompt.Tips.TipEntity;
...
public class TipList 
{
	...
	public static HashMap<String, Tip> onKillList= new HashMap<String, Tip>();
	...
	public static void ReadJsonTips(JsonObject jsonObject) 
	{
		...
		if(jsonObject.has(TipType.ON_ENTITY_KILL.toString()))
		{
			for(JsonElement element : jsonObject.get(TipType.ON_ENTITY_KILL.toString()).getAsJsonArray())
			{
				JsonObject data = element.getAsJsonObject();
				TipEntity tip = (TipEntity) new TipEntity(data.get("tipID").getAsString(), data.get("tipName").getAsString(), TipType.ON_ENTITY_KILL).ReadFromJson(data);
				
				if (tip == null) continue;
			    TIPS.add(tip);
			    idToTips.put(tip.tipId, tip);
			    onKillList.put(tip.entityID, tip);
			}

		}

		TipAlert.logger.log(Level.INFO, "Dimension Tips Loaded: "+ dimensionList.size());
		TipAlert.logger.log(Level.INFO, "Block Tips Loaded: "+ blockLookTipList.size());
		TipAlert.logger.log(Level.INFO, "ToolTips Loaded: "+ toolTipList.size());
		TipAlert.logger.log(Level.INFO, "General Tips Loaded: "+ (TIPS.size() - (blockLookTipList.size() + toolTipList.size() + dimensionList.size())) );
		TipAlert.logger.log(Level.INFO, "On Entity Killed Loaded: "+ onKillList.size() );		
		TipAlert.logger.log(Level.INFO, "Total Tips Loaded: "+ TIPS.size());
...
//		}
	...
	public static Tip checkTipLookingAt(IBlockState block)
	{
		...
		for(Entry<ItemStack, Tip> tip : blockLookTipList.entrySet())
		{
			...
			{
				if(TipAlert.tipManager.tipNotification.isInQueue(tip.getValue())) return null;
				return tip.getValue();
			}
	...
	public static Tip getAndCheckTips(TipType type, Block block)
	{
		for(Tip tip : TIPS)
		{
			if(tip.type == type)
			{
				...
				Block tipBlock = Block.getBlockFromItem(item);
				 
				
				boolean flag = checkOreDiconary(tip, new ItemStack(block));
	
				
				if(tipBlock != null && (tipBlock.equals(block) || flag))
				{
					if(TipAlert.tipManager.tipNotification.isInQueue(tip)) return null;
					return tip;
				}
			}
		}
	...
	public static Tip checkEntityDeath(String entityID)
	{
		
		if(onKillList.containsKey(entityID.toLowerCase()))
		{
			return onKillList.get(entityID.toLowerCase());
		}
